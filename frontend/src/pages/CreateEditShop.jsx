            ...
            <div className='max-w-xl w-full bg-white dark:bg-[#1e1e1e] shadow-2xl rounded-3xl p-8 md:p-10 border border-gray-100 dark:border-gray-800 transition-colors duration-300 mt-8'>
                ...
                <form className='space-y-6' onSubmit={handleSubmit}>
                    <div>
                        ...
                        <input type="text" placeholder='Enter Shop Name' className='w-full px-4 py-3 bg-gray-50 dark:bg-[#2a2a2a] border border-gray-200 dark:border-gray-700 dark:text-white rounded-xl focus:outline-none focus:ring-2 focus:ring-[#ff4d2d] transition-colors' onChange={(e) => setName(e.target.value)} value={name} required/>
                    </div>
                    
                    <div>
                        <label className='block text-sm font-bold text-gray-700 dark:text-gray-300 mb-2'>Storefront Image</label>
                    ...
                    <div className='grid grid-cols-1 md:grid-cols-2 gap-6'>
                        <div>
                           ...
                           <input type="text" placeholder='City' className='w-full px-4 py-3 bg-gray-50 dark:bg-[#2a2a2a] border border-gray-200 dark:border-gray-700 dark:text-white rounded-xl focus:outline-none focus:ring-2 focus:ring-[#ff4d2d]' onChange={(e) => setCity(e.target.value)} value={city} required/> 
                        </div>
                        <div>
                            <label className='block text-sm font-bold text-gray-700 dark:text-gray-300 mb-2'>State</label>
                            <input type="text" placeholder='State' className='w-full px-4 py-3 bg-gray-50 dark:bg-[#2a2a2a] border border-gray-200 dark:border-gray-700 dark:text-white rounded-xl focus:outline-none focus:ring-2 focus:ring-[#ff4d2d]' onChange={(e) => setState(e.target.value)} value={state} required/> 
                        </div>
                    </div>
                    
                    <div>
                        <label className='block text-sm font-bold text-gray-700 dark:text-gray-300 mb-2'>Full Address</label>
                        <input type="text" placeholder='Enter Complete Address' className='w-full px-4 py-3 bg-gray-50 dark:bg-[#2a2a2a] border border-gray-200 dark:border-gray-700 dark:text-white rounded-xl focus:outline-none focus:ring-2 focus:ring-[#ff4d2d]' onChange={(e) => setAddress(e.target.value)} value={address} required/> 
                    </div>
                    ...
                    <button className='w-full mt-4 bg-gradient-to-r from-[#ff4d2d] to-[#e64526] hover:from-[#e64526] hover:to-[#cc3e22] text-white px-6 py-4 rounded-xl font-bold text-lg shadow-lg hover:shadow-xl hover:-translate-y-1 transition-all duration-300 cursor-pointer flex justify-center items-center' disabled={loading}>
                        {loading ? <ClipLoader size={24} color='white' /> : "Save Restaurant Profile"}
                    </button>
                </form>
            </div>
        </div>
